...
--`repeater_users` float64,
--`repeater_users_pct` float64,
--`repeater_streams` float64,
`repeater_streams_pct` float64,
--`focused_users` float64,
--`focused_users_pct` float64,
--`focused_streams` float64,
`focused_streams_pct` float64,
--`avg_accounts_per_user` float64,
 	...
 	with main_query as (
		...
		past_week_streams as (select isrc, sum(number_of_streams) as streams
            ...
            f1_tracks as (select isrc
					...
					where streams > weekly_streams --threshold here
					group by isrc),
            ...
            onday_users_past_month_features as (select user_id, 
                                                ...
                                                from (select a.user_id, a.isrc, a.upc, a.number_of_streams, b.spinnup_artist_id, a.device_type
                                                      from (select * 
                                                            from `umg-spinnup.spotify.spotify_trends` 
                                                            where report_date between date_sub(sday, interval 27 day) and sday
            ...
            focused_users as (select user_id
					...
					where num_accounts >= thres_accounts --threshold here
		...
		repeat_users as (select user_id
					...
					where streams >= thres --threshold here
					group by user_id),
		...
		audio as (select a.isrc, b.duration, b.instrumentalness
            ...
            dataset as (select sday as report_date,
			a.isrc, 
			--a.streams,
			--a.users,
                  --a.streams_per_user,
