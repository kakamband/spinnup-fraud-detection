...
DIR = '{{ var.value.data_dir }}/spinnup_fraud_detection_v02{{ds_nodash}}' 
...
dag = DAG(dag_id = 'spinnup_fraud_detection_v02',
...
download_script = gcs_download_operator.GoogleCloudStorageDownloadOperator(
    ...
    object='builds/spinnup/fraud_detection/v02.py',
    filename='{}/v02.py'.format(DIR),
...
generate_results = BashOperator(
	...
	bash_command = 'cd {} && python v02.py'.format(DIR),
    env={'DIR': DIR, 'ds': ds},
	dag = dag
)
...
upload_to_gcs = file_to_gcs.FileToGoogleCloudStorageOperator(
    ...
    dst = 'data/spinnup/fraud_detection_v02/year={{ macros.ds_format(ds, \'%Y-%m-%d\', \'%Y\') }}/month={{ macros.ds_format(ds, \'%Y-%m-%d\', \'%Y%m\') }}/day={{ macros.ds_format(ds, \'%Y-%m-%d\', \'%Y%m%d\') }}/v02_results{{ds_nodash}}.csv',
    bucket = '{{ var.value.project_bucket }}',
    conn_id = 'google_cloud_default',
    src = '%s/v02_results.csv' % (DIR),
    dag = dag
)
...
load_to_bq = gcs_to_bq.GoogleCloudStorageToBigQueryOperator(
    task_id='load_to_bq',
    bucket='{{ var.value.project_bucket }}',
    source_objects=['data/spinnup/fraud_detection_v02/year={{ macros.ds_format(ds, \'%Y-%m-%d\', \'%Y\') }}/month={{ macros.ds_format(ds, \'%Y-%m-%d\', \'%Y%m\') }}/day={{ macros.ds_format(ds, \'%Y-%m-%d\', \'%Y%m%d\') }}/v02_results{{ds_nodash}}.csv'],
    destination_project_dataset_table='umg-data-science.detect_fraud_spinnup.v02_results${{ ds_nodash }}', 
    schema_object='builds/flow/schemas/spinnup/spinnup_fraud_detection_v02_schema.json',
    source_format='CSV',
    field_delimiter = ',',
    bigquery_conn_id = 'bigquery_default',
    skip_leading_rows=0,
...
generate_dashboard_data = bigquery_operator.BigQueryOperator(
    task_id='generate_dashboard_data',
    sql='sql/spinnup/fraud_detection_v02_dashboard.sql',
    destination_dataset_table='umg-data-science.detect_fraud_spinnup.v02_dashboard${{ ds_nodash }}',
    write_disposition='WRITE_TRUNCATE',
    allow_large_results=True,
    bigquery_conn_id='bigquery_default',
    use_legacy_sql=False,
...
# dag structure
spinnup_partition_sensor >> generate_dataset >> mkdir >> download_script >> generate_results >> upload_to_gcs >> load_to_bq >> delete_directory >> generate_dashboard_data